...
export class ReportsService {
  ...
  async getProfitAndLoss(userId: string, businessId: string, months = 12) {
    ...
    const expenseTransactions = await this.prisma.bizTransaction.findMany({ where: { businessId,
        type: 'EXPENSE',
        date: { gte: startDate },
      },
    ...
    const incomeTransactions = await this.prisma.bizTransaction.findMany({ where: { businessId,
        type: 'INCOME',
        date: { gte: startDate },
      },
      select: { amount: true, date: true },
